...
public class CheckmateDetector {
    ...
    public boolean whiteCheckMated() {
        ...
        // If yes, check if king can evade
        if (canEvade(bMoves, b.Wk))
            checkmate = false;

        // If no, check if threat can be captured
        List<Piece> threats = bMoves.get(b.Wk.getPosition().hashCode());
        if (canCapture(wMoves, threats, b.Wk))
            checkmate = false;

        // If no, check if threat can be blocked
        if (canBlock(threats, wMoves, b.Wk))
            checkmate = false;
        ...
        if (threats.size() == 1) {
            ...
            if (ks.getXNum() == ts.getXNum()) {
                ...
                for (int i = min + 1; i < max; i++) {
                    List<Piece> blks = blockMoves.get(brdArray[i][ks.getXNum()].hashCode());
            ...
            if (ks.getYNum() == ts.getYNum()) {
                ...
                for (int i = min + 1; i < max; i++) {
                    List<Piece> blks = blockMoves.get(brdArray[ks.getYNum()][i].hashCode());
                    ConcurrentLinkedDeque<Piece> blockers = new ConcurrentLinkedDeque<Piece>();
                    blockers.addAll(blks);
            ...
            if (tC.equals(Queen.class) || tC.equals(Bishop.class)) {
                ...
                if (kX > tX && kY > tY) {
                    for (int i = tX + 1; i < kX; i++) {
                        tY++;
                        List<Piece> blks = blockMoves.get(brdArray[tY][i].hashCode());
                        ConcurrentLinkedDeque<Piece> blockers = new ConcurrentLinkedDeque<Piece>();
                        blockers.addAll(blks);
